...
class RadConv(object):
    ...
    def __init__(self, **kwargs):
        ...
        self._rckeys = ['nlayer', 'niter', 'delta_t', 't_init', 'q_init',
                        'ocean_depth',
    ...
    def reset(self):
        ...
        for key, val in self._arrays.items():
            setattr(self, key, array(np.empty(0),
                                     var=key, name=val[0], unit=val[1]))
    ...
    def run(self):
        ...
        with open(os.path.join(PATH, 'radconv.nml'), 'w') as f:
            radconv_nml = ', '.join(['%s = %s' % (
                k, v) for k, v in self.__dict__.items()
                if k.lower() in self._rckeys])
        ...
        with open(os.path.join(PATH, 'radiation.nml'), 'w') as f:
            radiation_nml = ', '.join(['%s = %s' % (
                k, v) for k, v in self.__dict__.items()
                if k.lower() in self._rkeys])
        ...
        for arr in self:
            ...
            try:
                file = np.loadtxt(os.path.join(
                    PATH, 'output', '%s.dat' % arr.var))
            except:
                import pdb
                pdb.set_trace()
            setattr(self, arr.var, array(file,
                                         var=arr.var,
                                         name=arr.name, unit=arr.unit))
    ...
    def animate(self, *arrs):
        ...
        for i, arr in enumerate(arrs):
            convlabel[i] = ax[i].annotate('$\Delta$ = %.2f' % np.inf,
                                          xy=(0.95, 0.965), ha='right',
                                          va='top',
            ...
            if arr.var == 't':
                # Show the surface
                surft = ax[i].text(self.ts[0], self.ps[0], 'S',
                                   ha='center', va='center', fontsize=8,
                                   color='k')
                # Show the adiabat
                adiabat, = ax[i].plot(
                    self.t_ad[0], self.p[0], color='r', alpha=0.25)
                labelt = ax[i].text(self.t_ad[0, self.nlayer // 3],
                                    self.p[0, self.nlayer // 3],
                                    'Dry adiabat', fontsize=8,
                                    color='r', ha='right', va='top',
                                    alpha=0.5, clip_on=True)
            # Special case for water
            elif arr.var == 'q':
                # Show the saturation curve
                satur, = ax[i].plot(self.qsat[0], self.p[0],
                                    color='g', alpha=0.25)
                labelq = ax[i].text(self.qsat[0, self.nlayer // 3],
                                    self.p[0, self.nlayer // 3],
                                    'Saturation', fontsize=8,
        ...
        def updatefig(i):
            ...
            for j, arr in enumerate(arrs):
                ...
                if arr.var == 't':
                    ...
                    labelt.set_position(
                        (self.t_ad[i, self.nlayer // 3],
                         self.p[i, self.nlayer // 3]))
                elif arr.var == 'q':
                    satur.set_xdata(self.qsat[i])
                    satur.set_ydata(self.p[i])
                    labelq.set_position(
                        (self.qsat[i, self.nlayer // 3],
                         self.p[i, self.nlayer // 3]))
            return curve

        anim = animation.FuncAnimation(
            fig, updatefig, frames=len(self.p), interval=50, repeat=True)
